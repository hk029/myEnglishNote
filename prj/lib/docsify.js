(function () {
  ...
  if (script) {
    ...
    if (config.loadSidebar === true) {
      config.loadSidebar = '_sidebar.md';
    }
    if (config.loadNavbar === true) {
      config.loadNavbar = '_navbar.md';
    }
    if (config.coverpage === true) {
      config.coverpage = '_coverpage.md';
    }
    if (config.repo === true) {
      config.repo = '';
    }
  ...
  var marked = createCommonjsModule(function (module, exports) {
    ...
    (function () {
      ...
      InlineLexer.prototype.output = function (src) {
         debugger
        // add 这里把所有的解析过的inline元素（除去了### 这样块级标签）再进行处理
        var this$1 = this;

        var out = '',
          link, text, href, cap;
        // 循环对src进行处理，cao是用规则exec处理了字符串后的结果
        while (src) {
          // escape
          ...
          // b
          if (cap = this$1.rules.b.exec(src)) {
            src = src.substring(cap[0].length);
            out += this$1.renderer.b(this$1.output(cap[2] || cap[1]));
            continue;
          }
          // em
      ...
      //addd 增加==高亮
      Renderer.prototype.b = function (text) {
        return '<b>' + text + '</b>';
      };
  ...
  Compiler.prototype._initRenderer = function _initRenderer() {
    ...
    origin.link = renderer.link = function (href, title, text) {
      var blank = '';
      // console.log('orlink',curPath);
      console.log('orlink', href, curPath);
      if (!/:|(\/{2})/.test(href) &&
      ...
      // 增加.和..
      if (href.indexOf('.') >= 0 && window) {
        if (href.indexOf('..') < 0) {
          href = href.replace(/\/([s|n])\.\//g, function (find, $1) {
            return curPath[$1];
          })
        } else {
          href = href.replace(/\/([s|n])\.\.\//g, function (find, $1) {
            var level = curPath[$1].split('/').length;
            return curPath[$1].split('/').splice(0,level-2).join('/');
          })
        }
      }
  ...
  function highlight() {
    ...
    var tmp = getNavKey(location.hash.slice(1).replace(/\?.*/g, ''), last.getAttribute('data-id'));
    var li = nav[tmp];
    // console.log(tmp, 'nav', nav, 'li:', li, 'ac:', active, 'las', last);
  ...
  function loadNested(path, file, next, vm, first) {
    ...
    if (file.indexOf('side') >= 0) {
      curPath['s'] = path;
    } else if (file.indexOf('nav') >= 0) {
      curPath['n'] = path;
    }
    console.log('path', path);
    get(vm.router.getFile(path + file))
      .then(next, function (_) {
        return loadNested(path, file, next, vm);
      });
  }
  ...
  function fetchMixin(proto) {
    ...
    proto._fetch = function (cb) {
      ...
      // Load nav
      loadNavbar &&
        loadNested(path, loadNavbar, function (text) {
    ...
    proto._fetchCover = function () {
      ...
      get(path)
        .then(function (text) {
          return this$1._renderCover(text);
        });
    };
